...
def draw_roc_curve(ax, y_true: list, y_score: list, title, legend=True):
    colors = ['#bc02bb', '#3b63e4', '#74aed4', '#428c2d', '#f8ae55', '#8cf08d']
    labels = ['勾画者1', '勾画者2', '勾画者3', '勾画者4', '勾画者5', '勾画者6']
    roc_curves = []
    scatter_dots = []
    auc_labels = []
    ...
    if legend:
        ...
        italic_P = '$P$'
        legend = ax.legend(
            [roc_curves[0], roc_curves[1], roc_curves[2], roc_curves[3], roc_curves[4], roc_curves[5],
             (scatter_dots[0], scatter_dots[1], scatter_dots[2], scatter_dots[3], scatter_dots[4], scatter_dots[5]),
             baseline],
            [auc_labels[0], auc_labels[1], auc_labels[2], auc_labels[3], auc_labels[4], auc_labels[5],
             f'All {italic_P} >= ' + r'$\frac{0.05}{15}$' + ' (Bonferroni rectify)',
             '基准线'],
            loc='upper left',
...
if __name__ == '__main__':
    ...
    indices = ['outline_ResNet_p1', 'outline_ResNet_p2', 'outline_ResNet_p3', 'outline_ResNet_p4', 'outline_ResNet_p5',
               'outline_ResNet_p6']
    results_path_all = {
    ...
    df_path5 = pd.read_csv('.' + results_path_all[indices[4]])
    df_path6 = pd.read_csv('.' + results_path_all[indices[5]])

    y_true_list, y_score_list = get_data(df_path1, df_path2, df_path3, df_path4, df_path5, df_path6)

    draw_roc_curve(ax, y_true_list, y_score_list, '')
    add_zoom_axes(ax, (0.13, 0.77), (0.23, 0.865))

    plt.savefig('./saved_figs/Outline_ResNet_zh.tiff', bbox_inches='tight')
    # plt.show()

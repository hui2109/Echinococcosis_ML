...
def draw_roc_curve(ax, y_true: list, y_score: list, title, legend=True):
    colors = ['#bc02bb', '#3b63e4', '#74aed4', '#428c2d']
    labels = ['VGG19', 'EfficientNetB0', 'InceptionV3', 'ResNet50']
    roc_curves = []
    scatter_dots = []
    auc_labels = []
    ...
    if legend:
        ...
        italic_P = '$P$'
        ax.legend(
            [roc_curves[0], roc_curves[1], roc_curves[2], roc_curves[3],
             (scatter_dots[0], scatter_dots[1]),
             (scatter_dots[0], scatter_dots[2]),
             (scatter_dots[0], scatter_dots[3]),
             baseline],
            [auc_labels[0], auc_labels[1], auc_labels[2], auc_labels[3], f'{italic_P} < .001', f'{italic_P} < .001', f'{italic_P} < .001', '基准线'],
            loc='upper left',
...
if __name__ == '__main__':
    ...
    indices = ['DL_test', 'efficientNet_test', 'inception_test', 'ResNet_test']
    results_path_all = {
        ...
        'efficientNet_val': './resources/results/DL/efficientnetb0_rmsprop/validation.csv',
        'efficientNet_test': './resources/results/DL/efficientnetb0_rmsprop/test.csv',

        'inception_val': './resources/results/DL/inceptionv3_rmsprop/validation.csv',
        'inception_test': './resources/results/DL/inceptionv3_rmsprop/test.csv',

        'ResNet_val': './resources/results/DL/resnet50_adam/validation.csv',
        'ResNet_test': './resources/results/DL/resnet50_adam/test.csv',
    ...
    df_path4 = pd.read_csv('.' + results_path_all[indices[3]])

    y_true_list, y_score_list = get_data(df_path1, df_path2, df_path3, df_path4)

    draw_roc_curve(ax, y_true_list, y_score_list, '')
    add_zoom_axes(ax, (0.15, 0.75), (0.22, 0.9))

    plt.savefig('./saved_figs/Test_VGG_VS_Others_Zoom_zh.tiff', bbox_inches='tight')
    # plt.show()

...
def get_data(norm, fdr, fs, classifier, curr_num, val, path='all'):
    auc_values = []
    # lower_limits = []
    # upper_limits = []
    stds = []
    for i in range(1, 41):
        result_dict = open_specific_path(norm, fdr, f'{fs}_{i}', classifier, path)
    ...
    best_performance_minus_se = get_std(auc_values, norm, fdr, fs, classifier, path)

    return auc_values, lower_limits, upper_limits, best_performance_minus_se


def get_std(auc_values, norm, fdr, fs, classifier, path='all'):
    # 计算1-se
    best_auc = np.max(auc_values)
    best_auc_fm = np.argmax(auc_values) + 1
    result_dict = open_specific_path(norm, fdr, f'{fs}_{best_auc_fm}', classifier, path)
...
def draw_error_bar(ax, auc_values: np.array, lower_limits: np.array, upper_limits: np.array, title, curr_num,
    ...
    # 绘制当前选择的点
    line2 = ax.scatter(x[curr_num - 1], auc_values[curr_num - 1], color='red', label='当前模型', zorder=10, s=6.0)
    # 绘制误差图
    ...
    ax.set_xlabel('特征数', fontdict=font_dict)
...
if __name__ == '__main__':
    ...
    auc_values, lower_limits, upper_limits, best_performance_minus_se = get_data('Mean', 'PCC', 'KW', 'RF', 32, 0.873, 'all')
    draw_error_bar(ax, auc_values, lower_limits, upper_limits, '', 32, best_performance_minus_se, ['#bbd1e7', '#80a6e2'], '训练集', True)

    plt.savefig('./saved_figs/Feature_Number_New.jpg', bbox_inches='tight')
    plt.show()

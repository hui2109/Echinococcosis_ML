...
def draw_grouped_histogram(ax, species: list, data: dict, fig_title):
    ...
    font_dict = {
        'weight': 'bold',
        'size': 12}
    ax.set_xlabel('归一化', fontdict=font_dict)
...
if __name__ == '__main__':
    ...
    # 训练集及独立测试集
    source_1_Train = open_specific_path('Mean', 'PCC', 'KW_24', 'RF')
    source_2_Train = open_specific_path('Zscore', 'PCC', 'KW_24', 'RF')
    source_3_Train = open_specific_path('MinMax', 'PCC', 'KW_24', 'RF')

    source_1_Test = open_specific_path('Mean', 'PCC', 'Relief_32', 'RF', 'better')
    source_2_Test = open_specific_path('Zscore', 'PCC', 'Relief_32', 'RF', 'better')
    source_3_Test = open_specific_path('MinMax', 'PCC', 'Relief_32', 'RF', 'better')

    data_1 = {
        '训练集': (
            source_1_Train['cross-validation validation'], source_2_Train['cross-validation validation'],
            source_3_Train['cross-validation validation']),
        '独立测试集': (source_1_Test['testing'], source_2_Test['testing'], source_3_Test['testing'])
    }
    species = ['Mean', 'Z-score', 'MinMax']
    draw_grouped_histogram(ax, species, data_1, '')

    plt.savefig('./saved_figs/Normalization_New.jpg', bbox_inches='tight')
    plt.show()

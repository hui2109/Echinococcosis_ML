...
if __name__ == '__main__':
    ...
    # 训练集、交叉验证集及独立测试集
    source_1 = open_specific_path('Mean', 'PCA', 'Relief_32', 'RF')
    source_2 = open_specific_path('Mean', 'PCC', 'Relief_32', 'RF')
    data_1 = {
        '训练集': (
            source_1['train_AUC'], source_2['train_AUC']),
        '交叉验证集': (
            source_1['cross-validation validation'], source_2['cross-validation validation']),
        '测试集': (source_1['testing'], source_2['testing'])
    }
    ...
    plt.savefig('./saved_figs/Figure 2.tiff', bbox_inches='tight')
    plt.show()

...
def draw_roc_curve(ax, y_true: list, y_score: list, title, legend=True):
    colors = ['#bc02bb', '#3b63e4', '#74aed4']
    labels = ['RF', 'Senior-H', 'Senior-L']
    roc_curves = []
    scatter_dots = []
    auc_labels = []
    ...
    if legend:
        ...
        ax.legend(
            [roc_curves[0], roc_curves[1], roc_curves[2],
             ...
             baseline],
            [auc_labels[0], auc_labels[1], auc_labels[2], f'{italic_P} = .752', f'{italic_P} < .001', '基准线'],
            loc='upper left',
...
if __name__ == '__main__':
    ...
    indices = ['ML_RF_inves', 'External_senior_E', 'External_senior_NE']
    results_path_all = {
    ...
    add_zoom_axes(ax, (0.13, 0.74), (0.28, 0.92))

    plt.savefig('./saved_figs/ML_RF_VS_Inves_Zoom_zh.tiff', bbox_inches='tight')
    # plt.show()

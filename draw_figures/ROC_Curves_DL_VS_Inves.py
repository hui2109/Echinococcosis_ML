...
def draw_roc_curve(ax, y_true: list, y_score: list, title, legend=True):
    ...
    labels = ['VGG19', 'Senior-H', 'Senior-L']
    roc_curves = []
    scatter_dots = []
    auc_labels = []
    ...
    if legend:
        ...
        ax.legend(
            [roc_curves[0], roc_curves[1], roc_curves[2],
             ...
             baseline],
            [auc_labels[0], auc_labels[1], auc_labels[2], f'{italic_P} < .001', f'{italic_P} < .001', '基准线'],
            loc='upper left',
...
if __name__ == '__main__':
    ...
    indices = ['DL_inves', 'External_senior_E', 'External_senior_NE']
    results_path_all = {
    ...
    add_zoom_axes(ax, (0.02, 0.73), (0.3, 0.975))

    plt.savefig('./saved_figs/DL_VS_Inves_Zoom_zh.tiff', bbox_inches='tight')
    # plt.show()

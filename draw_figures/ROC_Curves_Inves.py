...
def draw_roc_curve(ax, y_true: list, y_score: list, title, legend=True):
    colors = ['#ee3432', '#3a63e4', '#cee000', '#000000', '#f7ae55', '#bc02bb', '#428c2d', '#8cf08d', '#74aed4']
    labels = ['所有超声医师', 'Senior-H', 'Middle-H', 'Junior-H', 'Mean HEA', 'Senior-L', 'Middle-L', 'Junior-L',
              'Mean LEA']
    roc_curves = []
    scatter_dots = []
    auc_labels = []
    for i in range(len(y_true)):
        ...
        scatter_dots.append(
            ax.scatter(fpr[closest_point_idx] - 0.01, tpr[closest_point_idx], marker='$*$', color=colors[i], zorder=3))
    ...
    if legend:
        ...
        ax.legend(
            [roc_curves[0], roc_curves[1], roc_curves[2], roc_curves[3], roc_curves[4], roc_curves[5], roc_curves[6],
             roc_curves[7], roc_curves[8],
             (scatter_dots[0], scatter_dots[1], scatter_dots[2], scatter_dots[3], scatter_dots[4], scatter_dots[5],
              scatter_dots[6], scatter_dots[7], scatter_dots[8]),
             baseline],
            [auc_labels[0], auc_labels[1], auc_labels[2], auc_labels[3], auc_labels[4], auc_labels[5], auc_labels[6],
             auc_labels[7], auc_labels[8], '当前分类器', '基准线'],
            loc='upper left',
...
if __name__ == '__main__':
    ...
    df_path1 = res_path = pd.concat([pd.read_csv('.' + results_path_all['External_senior_E']),
    ...
    df_path2 = pd.read_csv('.' + results_path_all['External_senior_E'])
    df_path3 = pd.read_csv('.' + results_path_all['External_middle_E'])
    df_path4 = pd.read_csv('.' + results_path_all['External_junior_E'])
    df_path5 = pd.concat([pd.read_csv('.' + results_path_all['External_senior_E']),
                          pd.read_csv('.' + results_path_all['External_middle_E']),
                          pd.read_csv('.' + results_path_all['External_junior_E']),
                          ], ignore_index=True)
    df_path6 = pd.read_csv('.' + results_path_all['External_senior_NE'])
    df_path7 = pd.read_csv('.' + results_path_all['External_middle_NE'])
    df_path8 = pd.read_csv('.' + results_path_all['External_junior_NE'])
    df_path9 = pd.concat([pd.read_csv('.' + results_path_all['External_senior_NE']),
                          pd.read_csv('.' + results_path_all['External_middle_NE']),
                          pd.read_csv('.' + results_path_all['External_junior_NE'])
                          ], ignore_index=True)

    y_true_list, y_score_list = get_data(df_path1, df_path2, df_path3, df_path4, df_path5, df_path6, df_path7, df_path8,
                                         df_path9)

    draw_roc_curve(ax, y_true_list, y_score_list, '')
    add_zoom_axes(ax, (0.14, 0.58), (0.37, 0.88))

    plt.savefig('./saved_figs/Inves_Zoom_zh.tiff', bbox_inches='tight')
    # plt.show()

...
if __name__ == '__main__':
    ...
    source_2_Train = open_specific_path('Mean', 'PCA', 'KW_24', 'RF')

    source_1_Test = open_specific_path('Mean', 'PCC', 'Relief_32', 'RF', 'better')
    source_2_Test = open_specific_path('Mean', 'PCA', 'Relief_32', 'RF', 'better')

    data_1 = {
        '训练集': (
            source_1_Train['cross-validation validation'], source_2_Train['cross-validation validation']),
        '独立测试集': (source_1_Test['testing'], source_2_Test['testing'])
    }
    species = ['PCC', 'PCA']
    draw_grouped_histogram(ax, species, data_1, '特征降维')

    plt.savefig('./saved_figs/Feature_Dimensionality_Deduction_New.jpg', bbox_inches='tight')
    plt.show()
